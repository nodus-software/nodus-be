...
func Enqueue(ctx context.Context, db Execer, message Message) error {
	...
	_, err := db.Exec(ctx, `INSERT INTO email_outbox
		(id,tenant_id,kind,recipient,recipient_hash,subject,text_body,html_body,expires_at)
		VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)`, message.ID, message.TenantID, string(message.Kind),
		recipient, hash, message.Subject, message.Text, message.HTML, message.ExpiresAt)
	return err
}
