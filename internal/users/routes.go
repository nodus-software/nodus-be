...
func (h *Handler) RegisterRoutes(r chi.Router) {
	r.Group(func(r chi.Router) {
		...
		r.With(middleware.RequirePermission("users:deactivate")).Post("/users/{userId}/suspend", h.SuspendUser)
		r.With(middleware.RequirePermission("users:deactivate")).Post("/users/{userId}/restore", h.RestoreUser)
		r.With(middleware.RequirePermission("users:read")).Get("/users/{userId}/security-status", h.GetSecurityStatus)
